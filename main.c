...
int main(int argc, char* argv[]) {
	...
	gtinit(type, 8, 0, 1, 0);		// initialize threads, see gthr.c
	gtgo(f, 6, 2, 20, 1);		// set f() as first thread
	gtgo(f, 4, 21, 80, 2);		// set f() as second thread
	gtgo(g, 2, 81, 85, 3);		// set g() as third thread
	gtgo(g, 0, 86, 100, 4);		// set g() as fourth thread
